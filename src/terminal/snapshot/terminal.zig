...
//! ```
//!
//! This is the upstream v1 mode registry. Yuurei appends the transport-specific
//! win32_input mode to native ModePacked, but does not persist that negotiation
//! in portable snapshots. Changing the wire registry requires a snapshot version
//! bump. Decoders ignore reserved bits, including the native fork-only bit.
//!
...
pub const Header = struct {
    ...
    pub fn encode(
        ...
        // Preserve the upstream v1 registry; ConPTY input negotiation is local
        // to the attached transport and is not part of the portable snapshot.
        ...
        for (mode_values) |value| {
            ...
            try io.writeInt(writer, u64, @as(WireModeBits, @truncate(bits)));
        }
    ...
    pub fn decode(reader: *std.Io.Reader) PayloadDecodeError!Header {
        ...
        for (&mode_values) |*value| {
            ...
            const bits: ModeBits = @as(WireModeBits, @truncate(raw));
            value.* = @bitCast(bits);
        }
...
test "TERMINAL snapshot excludes transport-specific input mode" {
    var header = test_header;
    header.current_modes.win32_input = true;
    header.saved_modes.win32_input = true;
    header.default_modes.win32_input = true;
    var encoded: [Header.len]u8 = undefined;
    var writer: std.Io.Writer = .fixed(&encoded);
    try header.encode(&writer);
    try std.testing.expectEqualSlices(u8, &test_header_fixture, writer.buffered());

    // Reserved wire bit 43 must not enable ConPTY input when decoding a
    // snapshot from another implementation (current, saved, and default sets).
    for ([_]usize{ 44, 52, 60 }) |offset| encoded[offset] |= 0x08;
    var reader: std.Io.Reader = .fixed(writer.buffered());
    const decoded = try Header.decode(&reader);
    try std.testing.expect(!decoded.current_modes.win32_input);
    try std.testing.expect(!decoded.saved_modes.win32_input);
    try std.testing.expect(!decoded.default_modes.win32_input);
}
