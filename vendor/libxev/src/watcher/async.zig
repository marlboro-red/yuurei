...
fn AsyncIOCP(comptime xev: type) type {
    return struct {
        ...
        /// The notification state, heap-allocated rather than stored
        /// inline. Async values are commonly copied by value across
        /// threads (Ghostty copies the renderer thread's wakeup Async
        /// into termio before the renderer thread ever calls wait()).
        /// The eventfd/mach-port implementations tolerate that because
        /// copies share the underlying kernel object; inline fields
        /// fork the state instead, so notify() through a pre-wait()
        /// copy latches a flag nobody ever reads and the notification
        /// is silently lost. One shared allocation gives this backend
        /// the same copy semantics as the fd-based ones.
        state: *State,
        ...
        pub fn wait(
            ...
            self.state.guard.lock();
            defer self.state.guard.unlock();
            ...
            if (self.state.wakeup) loop.async_notify(c);
        }
        ...
        pub fn notify(self: *Self) !void {
            ...
            if (self.state.waiter) |w| {
                w.loop.async_notify(w.c);
            } else {
                self.state.wakeup = true;
            }
        }
